...
it('handleChange updates the value', () => {
  const { getByTestId } = render(<TimeLineTabs />);
  const tabsElement = getByTestId('tab-1M');
  
  fireEvent.click(tabsElement);
  expect(tabsElement.classList).toContain("Mui-selected");
});
